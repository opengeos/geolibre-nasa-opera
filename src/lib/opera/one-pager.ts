...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .print-map-icon, .print-map-attribution { display:none; }
  ...
  @media print { @page { size: landscape; margin: 8mm; } body { background:#fff; } .toolbar { display:none; } .page { border:none; box-shadow:none; margin:0; max-width:none; } .grid { grid-template-columns:.78fr 1.75fr .92fr; padding-bottom:8px; } .panel { padding:10px; } .panel.bg { grid-column:1; grid-row:1; font-size:11px; line-height:1.3; } .panel.map { grid-column:2; grid-row:1; } .panel.impact-panel { grid-column:3; grid-row:1; } #assessment-map { min-height:310px; } #overview-map { position:relative; height:90px; margin-top:6px; } #overview-map > svg { visibility:hidden; } #overview-map::after { content:""; position:absolute; z-index:10; left:25%; top:25%; width:50%; height:50%; border:2px solid #1d4ed8; background:rgba(37,99,235,.2); -webkit-print-color-adjust:exact; print-color-adjust:exact; } .print-map-icon { position:absolute; right:7px; z-index:20; display:flex; width:30px; height:30px; align-items:center; justify-content:center; border-radius:4px; background:#fff; color:#1f2937; box-shadow:0 0 0 1px rgba(0,0,0,.1); -webkit-print-color-adjust:exact; print-color-adjust:exact; } .print-map-icon svg { width:22px; height:22px; } .print-map-icon.fullscreen { top:7px; } .print-map-icon.globe { top:75px; } .print-map-attribution { position:absolute; z-index:19; right:1px; bottom:1px; left:70px; height:18px; display:flex; align-items:center; justify-content:flex-end; padding:0 5px; overflow:hidden; background:rgba(255,255,255,.94); color:#334155; font-size:7px; white-space:nowrap; -webkit-print-color-adjust:exact; print-color-adjust:exact; } }
</style>
</head>
<body>
  <div class="page">
    ...
    <div class="grid">
      ...
      <div class="panel map" id="map-panel" data-widget="map">
        ...
        <div class="map-wrap">
          ${mapBlock}
          ${input.geoLibreProject ? `<span class="print-map-icon fullscreen" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M8 3H3v5M16 3h5v5M8 21H3v-5M16 21h5v-5"/></svg></span><span class="print-map-icon globe" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3c3 3.2 3 14.8 0 18M12 3c-3 3.2-3 14.8 0 18M5.5 7.5h13M5.5 16.5h13"/></svg></span>` : ""}
          ${input.geoLibreProject ? `<span class="print-map-attribution">NASA Earthdata GIS · OpenFreeMap · © OpenStreetMap contributors</span>` : ""}
    ...
    <div class="footer">
      ...
      <span class="disclaimer">This map was generated using OPERA GeoAI and is intended for informational purposes. Results and interpretations require expert validation before use for decision-making.</span>
    </div>
  </div>
${
